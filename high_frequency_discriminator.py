...
class High_Frequency_Discriminator(nn.Module):
    def __init__(self, n_feature):
        super(High_Frequency_Discriminator, self).__init__()
        ...
        self.fc = nn.Linear(512*14*14, 1)

    def forward(self, x):
        x = F.max_pool2d(F.relu(self.conv1(x)), (2, 2))  # (batch_size,128, x/2, x/2)
        x = F.max_pool2d(F.relu(self.conv2(x)), 2)  # (batch_size,256, x/2, x/2)
        x = self.dropout(x)
...
if __name__ == '__main__':
    input_channels = 3
    model = High_Frequency_Discriminator(input_channels)

...
if hostname in ['user-Precision-7920-Tower', 'dell-Precision-7960-Tower']:  # 3091 or A6000
    imagenet_folder = '/datasets/Imagenet2012/train'
elif hostname == 'ubuntu':  # 503
    imagenet_folder = '/datasets/ILSVRC2012/train'
elif hostname == 'R2S1-gpu':  # 5014
    imagenet_folder = '/datasets/ImageNet2012/train'

output_folder = '/datasets/ImageNet_50k_990c'
...
excluded_class = [24, 99, 245, 344, 471, 555, 661, 701, 802, 919]
excluded_class_n = []
...
# 以下代码即使设置了种子，每次运行也会得到不同的结果(后发现可能是在不同服务器跑的问题，待验证)
...
#     images = os.listdir(class_image_folder)
#
#     # 在内存中维护一个集合，用于存储已经复制到目标文件夹中的图片的文件名
#     copied_images = set(os.listdir(class_folder))
#
#     # 随机选择一张图片并复制到对应的类别文件夹中，如果该图片已经存在，则重新选择，直到选择到不存在的图片
#     while True:
#         selected_image = random.choice(images)
#
#         if selected_image not in copied_images:
#             source_image_path = os.path.join(class_image_folder, selected_image)
#             destination_image_path = os.path.join(class_folder, selected_image)
#             shutil.copy(source_image_path, destination_image_path)
#             break
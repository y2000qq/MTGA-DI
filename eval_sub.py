...
for idx, target in enumerate(targets):
    ...
    # Let's remove the samples of the target (we only left with 450 samples of other 9 classes)
    source_samples = []
    for img_name, label in test_set.samples:
        if label != target:
            source_samples.append((img_name, label))
    ...
    netG.load_state_dict(torch.load('pretrained_generators/netG_{}_{}_19_t{}.pth'.format(args.source_model,args.source_domain, target)))
    netG = netG.to(device)
    netG.eval()
